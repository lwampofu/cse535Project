...
eeg=pd.DataFrame(dataset)
live=pd.DataFrame(dataset['live'])
eeg= eeg.drop("live", axis='columns')
...
def generator (z, reuse =None):
    with tf.variable_scope(name_or_scope  = 'gen',reuse = reuse):
        ...
        output = tf.layers.dense(inputs = hidden2, units = 4800, activation = tf.nn.tanh)
        return output
...
real_images = tf.placeholder(tf.float32 ,shape =[128,4800])
z = tf.placeholder(tf.float32 ,shape =[128,1024])
...
with tf. Session () as sess:
    ...
    for e in range(epochs):
        ...
        for batch_images in group_list(eeg,128):
            x = batch_images
            count+=1
            # Z ( random latent noise for Generator )
            # -1 to 1 because of tanh activation
            batch_z = np.random.uniform(-1,1, size =(128, 1024))
...
# Get the generated data and add save it as a new dataset

samples_array = np.array(samples)
samples_df = pd.DataFrame(samples_array) 
final = live.join(samples_df)
final.to_csv('attack_gan.csv',index=False)    
